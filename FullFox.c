...
void multiply(int * a, int * b, int * c, int n) {// for performing mutiplication
  int i, j, k;
  for (i = 0; i < n; i++) {
    for (j = 0; j < n; j++) {
      for (k = 0; k < n; k++) {
...
void fillData(int * A, int * B, int * C, int blockSize) {// initialize the matrices
...
void printMatrices(int * A, int * B, int * C, int blockSize) { // prints the 3 matrices
  printf("\nProcess 0: MATRIX A\n");
  for (int i = 0; i < blockSize; i++) {
    for (int j = 0; j < blockSize; j++) {
...
void fillMatrix_Main(int * A, int * B, int * C, int size) {// to fill the main matrix
  int c = 1;
...
if(rank == 0){
  ...
  MPI_Cart_shift(commcol, 0, 1, & dest, & source);//calculate from where to receive and where to send, receive from below send up.

  //printf("\ncoords %d %d source dest %d %d process %d\n", coord_2d[0], coord_2d[1], source, dest, rank_2d);
  ...
  for (int stage = 0; stage < dim; stage++) {
   ...
   //if the root is equal to the the column number it will broadcast it’s block A and all others that are on the same row
   //they will receive the A in TempA and proceed multiplication.
  ...
  MPI_Gather(C, 1, MatrixBlockType, Main_C, 1, MatrixBlockType, 0, cartcomm);// gather the whole matrix on process 0
  end = _rdtsc();

  exp = end - start;

  if (rank == 0)
    printf("\n FullFox algo  \t\t\t %Ld cycles\n\n", exp - residu);
